...
public class AccessTokenServiceTests {
    ...
    public void setUp() throws Exception {
        ...
        defaultAttributes = new TokenDefaultAttributes(defaultTokenExpirationSeconds, cloudAuthExpirationSeconds,
                maxTokenExpirationSeconds, minTokenExpirationSeconds, dataCenterPrefix, isTrustedServer);
        tokenService = new DefaultAccessTokenService(defaultAttributes, mockTokenDao, mockClientDao, mockUserService,
    ...
    public void shouldGetTokenForClient() {
        ...
        EasyMock.expect(mockTokenDao.findTokenForOwner(EasyMock.eq(clientId), EasyMock.eq(clientId))).andReturn(token);
        EasyMock.replay(mockTokenDao);
    ...
    public void shouldNotGetTokenForClient() {
        ...
        EasyMock.expect(mockTokenDao.findTokenForOwner(EasyMock.eq(clientId), EasyMock.eq(clientId))).andReturn(null);
        EasyMock.replay(mockTokenDao);
    ...
    private Client getFakeClient() {
        Client client =
                new Client(clientId, ClientSecret.newInstance(clientSecret), clientName, clientInum, clientIname,
                        "RCN-123-456-789", ClientStatus.ACTIVE, "inum=@!FFFF.FFFF.FFFF.FFFF!EEEE.EEEE!1111", userInum);
        return client;
    }
}
