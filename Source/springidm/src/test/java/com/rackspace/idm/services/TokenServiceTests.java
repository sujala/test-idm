...
public class TokenServiceTests {
	...
	public void setUp() throws Exception {
		...
		try {
			config = new PropertiesConfiguration(dcPropsFile);
		...
		String dc_dfw_idm_location = config.getString("DC_DFW_IDM_LOCATION");
		String dc_london_idm_location = config
		...
		tokenService = new DefaultAccessTokenService(defaultAttributes,
				mockTokenDao, mockRefreshTokenDao, mockClientDao, mockUserDao,
				dcLocations, new StubLogger());
	}
	...
	public void shouldThrowErrorIfInumNullCreateAccessTokenForUser() {
		...
		EasyMock.expect(mockClientDao.findByClientId(clientId)).andReturn(
				getFakeClient());
		EasyMock.expect(mockUserDao.findByUsername(username)).andReturn(user);
		EasyMock.replay(mockClientDao, mockUserDao);
	...
	@Test
	public void shouldReturnUsernameFromUserToken() {
		AccessToken token = getFakeUserToken();
		EasyMock.expect(mockTokenDao.findByTokenString(tokenString)).andReturn(
				token);
		EasyMock.replay(mockTokenDao);
	...
	@Test
	public void shouldReturnNullFromClientToken() {
		AccessToken token = getFakeClientToken();
		EasyMock.expect(mockTokenDao.findByTokenString(tokenString)).andReturn(
				token);
		EasyMock.replay(mockTokenDao);

		String user = tokenService.getUsernameByTokenString(tokenString);
	...
	@Test
	public void shouldReturnNullForUsernameIfTokenIsNull() {
	...
	public void shouldRevokeToken() {
		...
		userToken.setRequestor(clientId);
		AccessToken clientToken = getFakeClientToken();
		clientToken.setOwner(clientId);
		...
		EasyMock.expect(mockUserDao.findByUsername(testUser.getUsername())).andReturn(
				testUser);
		EasyMock.replay(mockUserDao);
		...
		tokenService.revokeToken(clientToken.getTokenString(), userToken
				.getTokenString());
	...
	private AccessToken getFakeUserToken() {
		return new AccessToken(tokenString, tokenExpiration, tokenOwner,
				"requestor", IDM_SCOPE.FULL);
	}

	private AccessToken getFakeClientToken() {
		return new AccessToken(tokenString, tokenExpiration, clientInum,
				"requestor", IDM_SCOPE.FULL);
	}
	...
	private Client getFakeClient() {
		Client client = new Client(clientId, ClientSecret
				.newInstance(clientSecret), clientName, clientInum,
