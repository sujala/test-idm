...
#. For each role returned, must
   ...
   #. Role entries must be grouped via role and domain such that multiple 
      entries for the same role must exist for roles that are assigned to the
      user on tenants across multiple domains.
