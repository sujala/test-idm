...
class DefaultAnalyticsLoggerTest extends Specification {
    ...
    def "log gets the token expiration for authentication calls"() {
        when:
        analyticsLogger.log(new Date().getTime(), "authToken", null, null, "host", "userAgent", "POST", "tokens", 200, null, null, responseBody, responseType)

        then:
        analyticsLogHandler.log(_) >> { String json ->
            def message = gsonBuilder().fromJson(json, DefaultAnalyticsLogger.Message.class)
            def user = message.getAt("user")
            assert(user != null)
            assert(user.getAt("tokenExp") == "2012-04-13T13:15:00.000-05:00")
        }
    ...
    def "log gets the user when username is in request"() {
        given:
        def mockedUser = Mock(com.rackspace.idm.domain.entity.User)
        def username = "myuser"
        mockedUser.getUsername() >> username
        def requestBody = """{"user": {"username" : "${username}","email": "cmarin1-sub@example.com","enabled": true,"OS-KSADM:password":"Password1"}}"""
        userService.getUser(username) >> mockedUser

        when:
        analyticsLogger.log(new Date().getTime(), "authToken", null, null, "host", "userAgent", "POST", "users", 200, requestBody, 'application/json', null, 'application/json')
