...
public class JSONReaderForRaxAuthPolicy implements MessageBodyReader<Policy> {
    @Override
    ...
    public static Policy getPolicyFromJSONString(String jsonBody) {
        ...
        try {
            ...
            if (outer.containsKey(JSONConstants.RAX_AUTH_POLICY)) {
                JSONObject jsonPolicy = (JSONObject) parser.parse(outer.get(JSONConstants.RAX_AUTH_POLICY).toString());
