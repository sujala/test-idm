...
@Component
public class IdentityPropertyBooleanValueTypeValidator implements IdentityPropertyValueTypeValidator {

    public static final String VALUE_REQUIRED_MSG = "Identity property value required.";
    public static final String VALUE_INVALID_BOOLEAN_MSG = "Identity property value is an invalid boolean value.";

    @Override
    public boolean supports(IdentityPropertyValueType valueType) {
        return IdentityPropertyValueType.BOOLEAN == valueType;
    }

    @Override
    public void validateIdentityProperty(IdentityProperty identityProperty) {
        Validate.isTrue(IdentityPropertyValueType.BOOLEAN ==
        ...
        if (!Boolean.TRUE.toString().equalsIgnoreCase(identityProperty.getValue()) &&
                !Boolean.FALSE.toString().equalsIgnoreCase(identityProperty.getValue())) {
            throw new BadRequestException(VALUE_INVALID_BOOLEAN_MSG);
        }
    }
