...
class VerifyPhonePinForFedUserIntegrationTest extends RootIntegrationTest {
    ...
    def "SAML assertion 2.0 - Verify phone pin for a federated user; media = #accept"() {
        ...
        com.rackspace.idm.domain.entity.PhonePin phonePinEntity = phonePinService.checkAndGetPhonePin(fedUser)
        ...
        def response = cloud20.verifyPhonePin(utils.getIdentityAdminToken(), fedUser.id, phonePin)

        then:
        assert response.status == SC_NO_CONTENT

        when: "verify phone pin with SAML auth token"
        response = cloud20.verifyPhonePin(authResponse.token.id, fedUser.id, phonePin)
        ...
        response = cloud20.verifyPhonePin(utils.getToken(userAdmin.username), fedUser.id, phonePin)
        ...
        response = cloud20.verifyPhonePin(utils.getIdentityAdminToken(), fedUser.id, emptyPhonePin)
        ...
        response = cloud20.verifyPhonePin(utils.getIdentityAdminToken(), fedUser.id, incorrectPhonePin)

        then:
        IdmAssert.assertOpenStackV2FaultResponse(response, BadRequestFault, HttpStatus.SC_BAD_REQUEST, "Error code: 'PP-001'; Incorrect phone pin for userId: '"+fedUser.id+"'")
    ...
    def "SAML assertion 2.0 - Verify phone pin on federated user returns 404 when phone pin is not assigned during saml assertion"() {
        given:
        ...
        when:
        def response = cloud20.verifyPhonePin(utils.getServiceAdminToken(), fedUser.id, phonePin)
    ...
    def "SAML assertion 1.0 - Verify phone pin on federated user returns 404 when phone pin is not assigned during saml assertion"() {
        given:
        reloadableConfiguration.setProperty(IdentityConfig.FEATURE_ENABLE_PHONE_PIN_ON_USER_PROP, false)
        def username = testUtils.getRandomUUID("userAdminForSaml")
